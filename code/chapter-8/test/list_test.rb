...
describe 'BYORedis - List commands' do
  ...
  describe 'RPOPLPUSH' do
    ...
    it 'works with the same list, even if it has a single element' do
      assert_command_results [
        [ 'LPUSH source a', ':1' ],
        [ 'LRANGE source 0 -1', [ 'a' ] ],
  ...
  describe 'BLPOP' do
    ...
    it 'returns an error if the last argument is not an integer or a float' do
      assert_command_results [
        ...
        [ 'BLPOP a inf', '-ERR timeout is negative' ],
        [ 'BLPOP a +inf', '-ERR timeout is negative' ],
        [ 'BLPOP a infinity', '-ERR timeout is negative' ],
        [ 'BLPOP a +infinity', '-ERR timeout is negative' ],
        [ 'BLPOP a -infinity', '-ERR timeout is negative' ],
        [ 'BLPOP a -inf', '-ERR timeout is negative' ],
      ]
    ...
    it 'clears the blocked states after a timeout' do
      with_server do |server_socket|
        ...
        response = read_response(server_socket, read_timeout: 0.3)
        duration = Time.now - start_time
  ...
  describe 'BRPOP' do
    ...
    it 'returns an error if the last argument is not an integer or a float' do
      assert_command_results [
        ...
        [ 'BRPOP a inf', '-ERR timeout is negative' ],
        [ 'BRPOP a +inf', '-ERR timeout is negative' ],
        [ 'BRPOP a infinity', '-ERR timeout is negative' ],
        [ 'BRPOP a +infinity', '-ERR timeout is negative' ],
        [ 'BRPOP a -infinity', '-ERR timeout is negative' ],
        [ 'BRPOP a -inf', '-ERR timeout is negative' ],
      ]
  ...
  describe 'BRPOPLPUSH' do
    ...
    it 'returns an error if the last argument is not an integer or a float' do
      assert_command_results [
        ...
        [ 'BRPOPLPUSH a b inf', '-ERR timeout is negative' ],
        [ 'BRPOPLPUSH a b +inf', '-ERR timeout is negative' ],
        [ 'BRPOPLPUSH a b infinity', '-ERR timeout is negative' ],
        [ 'BRPOPLPUSH a b +infinity', '-ERR timeout is negative' ],
        [ 'BRPOPLPUSH a b -infinity', '-ERR timeout is negative' ],
        [ 'BRPOPLPUSH a b -inf', '-ERR timeout is negative' ],
      ]
  ...
  def assert_blocking_behavior(blocking_command:, list_names:, timeout:, push_commands:,
    ...
    with_server do |socket1|
      ...
      response2 = read_response(socket1, read_timeout: 0.2)
