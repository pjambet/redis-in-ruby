...
describe BYORedis::SortedArray do
  ...
  describe 'push/<<' do
    it 'appends elements while keeping the array sorted' do
      sorted_array = BYORedis::SortedArray.by_fields(:timeout)
    ...
    it 'works with a more complicated comparison block' do
      sorted_array = BYORedis::SortedArray.by_fields(:timeout, :a)

      sorted_array << TestStruct.new('a', 1)
      sorted_array << TestStruct.new('zero', 0)
      sorted_array << TestStruct.new('z', 26)
      sorted_array << TestStruct.new('zz', 26)

      assert_equal(0, sorted_array[0].timeout)
      assert_equal('zero', sorted_array[0].a)
      assert_equal(26, sorted_array[-1].timeout)
      assert_equal('zz', sorted_array[-1].a)
    end
  end
